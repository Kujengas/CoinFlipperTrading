...
{
    public class DefaultStrategy : ITradingStrategy
    {
        ...
        public TradeDecision ShouldBuy(FlipperDataModel fdm)
        {
            ...
            if (points >= 3 && fdm.stats.CurrentBalance > 10)
            {
                ...
                return (new TradeDecision { doTrade = true, useMarket = false });
            }
            ...
            switch (fdm.algoConfig.StopLossMode)
            {
                ...
                case StopMode.Any:
                    ...
                    if (fdm.openOrders.orders != null)
                    {
                        ...
                        {
                            if (o.rate * fdm.algoConfig.StopLossPercentage >= te.buy || fdm.stats.LastOrderDate <= DateTime.Now.AddMinutes(-fdm.algoConfig.StopLossTime))
                            {
                                points += 4;
                            }


                            if (te.buy * fdm.algoConfig.StopLossPercentage >= o.rate || fdm.stats.LastOrderDate <= DateTime.Now.AddMinutes(-fdm.algoConfig.StopLossTime))
                            {
                                points += 4;
                            }
                ...
                case StopMode.Both:
                    if (fdm.openOrders.orders != null)
                    {
                        foreach (Order o in fdm.openOrders.orders)
                        {
                            ...
                            if (te.buy * fdm.algoConfig.StopLossPercentage >= Convert.ToDecimal(o.rate) && fdm.stats.LastOrderDate <= DateTime.Now.AddMinutes(-fdm.algoConfig.StopLossTime))
                            {
                                points += 4;
                            }
                ...
                case StopMode.Percentage:
                    if (fdm.openOrders.orders != null)
                    {
                        foreach (Order o in fdm.openOrders.orders)
                        {
                            if ((o.rate * fdm.algoConfig.StopLossPercentage) >= te.buy)
                            {
                                points += 4;
                            }


                            if (te.buy * fdm.algoConfig.StopLossPercentage >= o.rate)
                            {
                                points += 4;
                            }
